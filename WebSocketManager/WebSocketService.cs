...
public abstract class WebSocketServiceBase(ILogger logger, IWebSocketReceivingMessageHandler webSocketReceivingMessageHandler)
{
    ...
    public async Task InitializeSocket(WebSocket socket, CancellationToken stoppingToken)
    {
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                switch (result.MessageType)
                {
                    ...
                    case WebSocketMessageType.Binary:
                        break;
                    default:
                        logger.Debug("{MessageType}", result.MessageType);
                        break;
                }
            }
        }
        ...
        {
            logger.Debug(e, "Error Receiving Message");
            throw;
        }
    }
    ...
    protected async Task SendMessageAsync(WebSocket webSocket, string message)
    {
        ...
        using (LogContext.PushProperty("message", message))
        {
            ...
            {
                logger.Warning("Socket is not open!");
                return;
            }
            ...
            {
                logger.Warning(e, "Problem with sending message!");
            }
        }
    }
    ...
    private async Task OnConnected(WebSocket socket)
    {
        ...
        logger.Information("A connection is established in server with {SocketId}", socketId);
    ...
    {
        var socketId = WebSocketConnectionManager.GetId(socket);
        logger.Information("A connection is closed in server with {SocketId}", socketId);
    ...
    private async Task ReceiveAsync(WebSocket socket, WebSocketReceiveResult result, ArraySegment<byte> buffer, CancellationToken stoppingToken)
    {
        ...
        if (webSocketReceivingMessageHandler != null)
        {
            await webSocketReceivingMessageHandler.HandleAsync(message, stoppingToken);
        }
